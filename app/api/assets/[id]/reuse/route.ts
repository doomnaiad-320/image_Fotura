...
export async function POST(
  ...
  try {
    ...
    // 4. 获取当前复用积分配置
    const pointsConfig = await getReusePointsConfig();
    const chargeAmount = pointsConfig.current;
    ...
    return NextResponse.json({
      ...
      message: result.rewardGranted
        ? `复用成功！已扣除 ${chargeAmount} 积分，原作者获得 ${chargeAmount} 积分奖励`
        : `复用成功！已扣除 ${chargeAmount} 积分`
    });
  } catch (error: any) {
