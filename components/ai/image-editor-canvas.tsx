...
const ImageEditorCanvas: React.FC<ImageEditorCanvasProps> = ({ 
  ...
  const doDraw = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    if (maskCtx) {
      maskCtx.beginPath();
      // Use orange color for mask
      maskCtx.strokeStyle = '#f97316b3'; // 70% opacity orange
    ...
    <div className="flex flex-col gap-4">
        ...
        >
            {!initialImageUrl ? (
                ...
                <>
                    <button onClick={onClearImage} className="absolute top-2 right-2 z-30 p-1 bg-black/50 backdrop-blur-sm rounded-full text-white hover:bg-red-600 transition-colors" aria-label="移除图片">
        ...
        {initialImageUrl && isMaskToolActive && (
            <div className="p-3 bg-black/60 backdrop-blur-md rounded-lg flex flex-col gap-4 border border-white/10">
                <p className="text-xs text-gray-300 -mb-2">涂选要编辑的区域</p>
                <div className="flex items-center gap-4">
                    <label htmlFor="brush-size" className="text-sm font-medium text-white whitespace-nowrap">笔刷大小</label>
                    <input 
                        id="brush-size" 
                        type="range" 
                        min="5" 
                ...
                <div className="grid grid-cols-2 gap-2">
                    <button 
                        ...
                        className="px-4 py-2 text-sm font-semibold text-white bg-gray-600/40 rounded-md hover:bg-gray-600/60 disabled:bg-gray-800 disabled:text-gray-500 disabled:cursor-not-allowed transition-colors"
                    >
                        撤销
                    </button>
                    <button 
