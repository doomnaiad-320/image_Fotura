...
export function MessageItem({
  ...
  return (
    <div
      id={`msg-${message.id}`}
      className={`flex animate-in fade-in slide-in-from-bottom-4 duration-300 justify-start`}
        ...
        <div className="flex items-start gap-2">
          ...
          <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${isUser
            ...
            <span className="text-[12px] font-semibold">
              {isUser ? (userAvatarInitial || 'U') : modelInitial}
            </span>
          </div>

          {/* 名称与时间 */}
          <div className="flex flex-col items-start gap-0.5">
            <div className={`text-[12px] font-medium leading-tight ${isUser ? 'text-foreground' : 'text-orange-400'
              }`}>
              {isUser ? (userDisplayName || '用户') : modelName}
            </div>
        ...
        >
          ...
          >
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(80vh-80px)]">
              <pre className={`text-sm leading-relaxed whitespace-pre-wrap ${isJSON(promptText)
