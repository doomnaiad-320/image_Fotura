...
export type ReusedThumbGridProps = {
  initialItems: AssetListItem[];
  isAuthenticated: boolean;
};

export function ReusedThumbGrid({ initialItems, isAuthenticated }: ReusedThumbGridProps) {
  ...
  const onToggleFavorite = useCallback(async (assetId: string, nextState: boolean) => {
    ...
    setItems((prev) => prev.map((i) => (i.id === assetId ? { ...i, isFavorited: nextState } : i)));
